...
public class CineService {
    ...
    public Cine crearCine(Pelicula pelicula, SalaCine salaCine, double precio) {
        return new Cine(pelicula, salaCine, precio);
    }
    ...
    public void venderButaca(Cine cine, SalaCine salaCine, Espectador espectador) {
        //Validar que el espectador tenga la edad mínima
        boolean mayorEdad = espectador.edad() >= salaCine.getPelicula().getEdadMinima();
        if (!mayorEdad) {
            System.out.println("Espectador menor de edad");
        }
        //Validar que el espectador tenga dinero suficiente
        boolean dineroSuficiente = espectador.dineroDisponible() >= cine.getPrecio();
        if (!dineroSuficiente) {
